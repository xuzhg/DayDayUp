...
int myAtoi(const char * pStr){
    if (pStr == nullptr) throw std::invalid_argument("null string input");
    ...
    if (*pStr == '\0') throw std::invalid_argument("empty string input");
    ...
    // make sure it's not an empty or whitespace only string
    if (*pStr == '\0') throw std::invalid_argument("wrong input");

    const int MaxInt1_10 = 0x7FFFFFFF;
    const int MinInt1_10 = 0x80000000;
    int result = 0;
    while (*pStr != '\0'){
        char ch = *pStr;
        ++pStr;
